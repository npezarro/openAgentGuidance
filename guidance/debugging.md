...
Second gotcha when your reader writes to the macOS clipboard: an app that pastes via the clipboard often does save, paste, then **restore**, clobbering your write. Re-assert (re-`pbcopy`) for ~1s, checking `pbpaste`, to win the race.
