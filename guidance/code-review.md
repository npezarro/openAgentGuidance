...
- **PM2-style `ecosystem.config.js`**: `env`, `max_memory_restart`, `cwd`, essential for production process management.
...
Never expose raw error messages, stack traces, internal paths, hostnames, or database connection strings in HTTP responses. This is OWASP "Improper Error Handling" and is one of the most common findings in a multi-repo audit.
...
**Why:** an "open in browser" helper passed user-controlled URLs through `exec()`. Fixed by switching to `execFile()` plus a URL validation guard rejecting non-http(s) protocols.
...
**Why:** a shell wrapper was silently producing malformed Python when scraped article titles contained special characters. Temp files eliminate all shell escaping concerns.
...
**Example:** a `head -c 2000` on CLI output truncated the structured marker block that a downstream notification step depended on. The script ran without errors but produced empty summaries for weeks.
...
**Why:** hard format constraints are enforcement gates for downstream parsers. Identifying a violation is not fixing it, and a response that diagnoses its own violation while still committing it is still a failure.
...
Real shape: a personal-finance tracker compiled `new RegExp(template.merchantPattern)` from stored template strings at three sites with no guard, inside a detect-all function that looped mappings with no try/catch. One malformed pattern threw SyntaxError and 500'd the detection endpoint, killing detection for ALL of the user's records. Same shape seen elsewhere: `JSON.parse` on index/metadata files without try/catch; waypoint interpolation `alpha=(t-t0)/(t1-t0)` with no guard for duplicate timestamps. Self-review trigger: any `new RegExp(non-literal)`, `JSON.parse(file/network)`, or division by a data-derived value inside a loop → ask "does one bad input abort the whole batch?". Bonus: compile invariant regexes once before the loop, not per-iteration.
...
Real shape: `salary: salary || data.salaryRange ? formatSalaryRange(data.salaryRange) : undefined` evaluated `formatSalaryRange(undefined)` → null whenever a text-extracted salary existed but the structured range didn't, so the Zod parse (`salary` is `z.string().optional()`, rejects null) threw and the record was silently dropped (catch → null → filter). Reviewer checklist: (1) any `x || y ? ... : ...` or `x && y ? ... : ...` in a value position is suspect, add parens or split it; (2) enable eslint `no-mixed-operators` and `no-unneeded-ternary` (the repo's eslint config did not flag this); (3) a sibling correct form nearby is a strong tell (the adjacent module used `salary || undefined`; the broken one was the outlier). Fix pattern: `salary || formatSalaryRange(range) || undefined`, coalesce to undefined so the field is never null.
...
Real shape: an extraction routine peeled a leading monologue plus a trailing handoff sentence off the FRONT, then treated everything from the artifact's first heading onward as the content, verbatim. It never re-scanned the tail. A separate refinement pass appended `## Editor Notes` with its own QA log at the END. Result: 69 of 151 completed artifacts (46%) shipped the model's internal QA log as a visible, user-facing section, and as a section-rail navigation anchor, making it look intentional.
...
- Measure against the full corpus, not a hand-picked sample: the defect here was invisible in most reads because whether the trailing section rendered depended on which run generated the artifact, not on anything visible in the leaking artifact itself.
...
`const limit = Math.min(Number(url.searchParams.get("limit") || 50), 200)` reads like input validation because it caps one thing (the max), but `Number()` on anything non-numeric produces `NaN`, and nothing here rejects `NaN`, negative numbers, or fractions before they reach the database layer. On Prisma 7.7 + `@prisma/adapter-libsql`, the failure modes for an unvalidated `take`/`skip` are NOT symmetric, which is what makes this easy to under-test:
