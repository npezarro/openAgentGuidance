...
- **Auth libraries (NextAuth/Auth.js and equivalents)**: `basePath`, `redirectProxyUrl`, provider `authorization.params`, `token.params` are required for subpath deployments behind reverse proxies.
- **Process-manager ecosystem config**: `env`, `max_memory_restart`, `cwd` are essential for production process management.
...
| ORM client cached only in dev (`globalFor*` pattern) | Connection leak in production | Cache on `globalThis` unconditionally (see below) |
...
Seen in the wild: an "open in browser" helper passed user-controlled URLs through `exec()`. The fix is `execFile()` plus a URL validation guard rejecting non-http(s) protocols.

## ORM Client Singleton: Always Cache in Production

The common Next.js ORM-client pattern only caches the client in development:
...
**Why:** a shell runner was silently producing malformed Python when scraped article titles contained special characters. Temp files eliminate all shell escaping concerns.
...
**Why:** a market-hours check used a hardcoded EDT offset, causing zero executions during EST months. The cron schedule was wrong the same way, with UTC hours interpreted as local time.
...
**Example:** a byte limit on CLI output truncated the structured block that downstream parsing depended on. The script ran without errors but produced empty summaries for weeks.
...
**Why:** it is common to violate this pattern and then self-diagnose the violation inside the same response, which demonstrates the rule was known and still not applied. Hard format constraints are enforcement gates for downstream parsers. Identifying a violation is not fixing it.

## Update CLAUDE.md When Adding Features

After implementing a new feature, route, export, or command, update the repo's CLAUDE.md before committing. Documentation lag is structural; close it at commit time. A drift-check hook that flags commits adding exports, routes, or env vars without a CLAUDE.md update makes this enforceable.
...
**Where to look:** Any `fixed` or `fixed inset-0` element inside a component that uses `backdrop-blur-*`, `blur-*`, `filter`, or CSS `transform`. Applies across any codebase using a shared glass-morphism card design system.
...
**Real shape:** a pace calculation guarded only `distanceM === 0`, so `computePace(8000, -100)` returned −12.5 (an invalid negative pace) which propagated into an average. A sibling adapter in the same codebase already used `speed <= 0` correctly; the two were inconsistent.
...
**Real shape:** an app compiled `new RegExp(storedPattern)` from stored template strings at three sites with no guard, inside a function that looped mappings with no try/catch. One malformed pattern threw `SyntaxError` and 500'd the endpoint, killing the feature for every record. Same shape elsewhere: `JSON.parse` on index/metadata files without try/catch; waypoint interpolation `alpha=(t-t0)/(t1-t0)` with no guard for duplicate timestamps.
...
**Real shape:** a profanity list contained a three-letter token matched via `any(p in all_text for p in WORDS)`. Every candidate containing pass/class/assist/massive/password/grass/compass/classic was flagged; because the scorer returns 0 for flagged items under a threshold, clean content was dropped from selection entirely.
...
- For any matching change, mechanically diff old vs new over a few hundred strings drawn from BOTH classes, rather than reasoning about which cases changed. The losses were invisible to inspection and obvious to a diff.
...
**Real shape:** a normalized action space scaled a symmetric `[-1,1]` action by one limit on both signs, making a separately-modeled larger limit unreachable for the agent-controlled entity. The commit message claimed the limit "could never bind"; a verifier found scripted non-agent entities reach the same model through a different, unnormalized path and clamp against that limit several times per episode. The defect was real, the sweeping claim was not. A reviewer who knows about the other caller reads the overreach as evidence you did not look.
...
A loader had been silently dropping several of a model's tunable limits. The fix forwarded them and coerced with `float()`. An independent verifier found that this made one case strictly WORSE than the bug it fixed: YAML resolves `yes/no/on/off` to Python bools, and `float(True)` is `1.0`. So `max_accel: yes`, previously ignored (leaving a sane default), now quietly installed a `1.0` limit with no error.
...
Two real instances from a single run:
- "This table has no indexes beyond the PK" was false. The ORM schema file declared no `index()`, which is what the note was based on, but the DB setup module created the single-column FK and timestamp indexes in a raw DDL string, which the test-db factory also executed. The indexes existed; only the COMPOSITE variants were missing.
- "These three exported functions have zero direct test coverage" was false: the matching test file already had 11 direct tests across all three.
...
**Corollary worth its own grep:** an unused-import lint error at a fix site is a signpost, not lint noise. "X is defined but never used" at the exact line a fix touched usually means that file stopped sharing something with its siblings. In the real case the unused import had CI red on the default branch for three weeks, starting with the fix commit itself. Reading only the LATEST failing run's date understates the outage; page back to the first failure before quoting a start date.
