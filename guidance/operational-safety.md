...
2. The agent finishes changes and runs `pm2 restart <service>` (or your deploy script)
...
1. **Hard guard in the deploy/restart script:** The `deploy` and `restart` verbs check the persisted job state for active jobs before restarting the service. If jobs are active, the command is refused with an error. This is the primary barrier.

2. **Prompt-level warning in the executor:** When a job's working directory is inside the service's own repo, prepend a self-restart-guard message to the prompt telling the agent not to restart the service. This is a soft barrier (the agent can ignore it).
...
3. **Existing deploy-script guard:** Still in place as a third layer, refusing to restart if active jobs exist. But since the process manager force-kills after SIGINT, this guard only works when the process can actually be signaled gracefully.
...
A restart storm is when a process enters a rapid restart cycle (restarts > 5 in under 5 minutes).
...
Observed: this exact bug made a scheduled security scanner silently fail for 13 consecutive days. It was detecting secrets daily but crashing before it could report findings. The state file never updated, so it rescanned the same repos with the same silent crash every run.
...
- **Syntax destruction.** Replacing a bare word with a multi-word phrase broke a shell `case` pattern, because `some phrase with spaces/*` is not a valid glob branch. The file stopped parsing.
- **Prose damage that survives review.** The same replacement rewrote a repo name *inside a path*, so a user-facing error told people to consult a path containing spaces and nonsense. The gate still worked; its instructions were gibberish, and nothing automated flags an absurd string.
...
Observed: a research collector spawned Claude for deep research. The top-level runner had `--dangerously-skip-permissions`, but the inner subprocess call didn't. Every research request's WebSearch calls were silently blocked, producing reports without web data.
...
**Also required: `--no-chrome`** for headless environments. The CLI may attempt to open a browser (e.g., for OAuth or a dashboard). In headless VMs or process-manager-supervised processes, this silently hangs or errors. Add `--no-chrome` alongside `--dangerously-skip-permissions` for all automated invocations:
...
Observed: a service used `execSync('claude -p --model <model-id>', { input: prompt })`. Every eval call passed the model flag string as the prompt instead of the real data.
...
> **Note:** this rule was originally written under the belief that inherited `CLAUDE_CODE_*` env vars caused a synthetic-401 incident. **That diagnosis was wrong.** Follow-up isolated testing (full polluted env including `CLAUDE_CODE_EXECPATH`, `CLAUDECODE=1`, and a dead `CLAUDE_CODE_SESSION_ID`) returned `is_error:false`. The true cause of those 401s was an OAuth refresh job being **rate-limited for several consecutive cron cycles**, leaving an expired access token. See "OAuth Refresh Rate-Limiting" below. The env-strip pattern is kept here as **defensive hygiene only**; it is not the fix for that incident.
...
**Why a process manager captures these vars:** It captures the full env at daemon start (including any `CLAUDE_CODE_*` vars from the terminal session that ran the restart). The vars persist in the process table for the lifetime of that process slot, even across subsequent restarts, until the process manager itself is restarted from a clean environment.

**Also strip `NODE_CHANNEL_FD`** when launching non-Node subprocesses from a Node.js parent (e.g., a Python worker called from an Express service). Node.js IPC sets `NODE_CHANNEL_FD` in its own env; child processes that themselves use Node runtimes (such as a JS challenge solver) inherit this FD reference and can fail with IPC errors because the FD is already closed or invalid in the new process.

Observed: a Python pipeline ran inside a Node.js parent. A child tool's node-based challenge solver inherited `NODE_CHANNEL_FD` and errored with IPC channel failures. Fix: strip it before spawning:
...
**The scenario:** a token-refresh script runs every 3h via cron and calls the OAuth token endpoint with a `refresh_token` grant. The endpoint is **rate-limited**, and under load can return `rate_limit_error: Rate limited. Please try again later.` for multiple consecutive cron cycles.
...
**Mitigations** (implement all three in the refresh script):
...
3. **Consecutive-cycle failure counter**, stored in a cache dir. After >=2 consecutive failures, post an alert to your notification channel with hours-remaining context. Counter resets on any successful or healthy cycle.
...
**Layered defense, browser path as safety net:** the cron refresh path and the browser-based re-login path are independent recovery mechanisms. When the OAuth API endpoint is rate-limiting (the cron path fails), the browser-based path completes `claude auth login` via the web OAuth flow, bypassing the API endpoint entirely. Validated in production: two consecutive runs exhausted all retries with `rate_limit_error`; the browser re-login chain then ran `claude auth login` through a browser agent, sidestepping the rate-limited endpoint; the next cron cycle ran clean with the counter reset to 0.

**Implication:** When debugging a prolonged OAuth failure, check both paths. If the cron log shows persistent `rate_limit_error` and the access token is expired, the recovery path is NOT to wait; it is to trigger the browser-based re-login.
...
Observed: one container's re-login failed with "callback tab not found" while sibling containers running the identical script ten minutes later succeeded. The fix: add `sleep 4` after locating the consent tab, then retry the click once if no callback appears within 25s.
...
### Auth-Age Enforcement, All Bridges Fail Simultaneously

**Symptom:** All automated container relogins fail on the same night with `callback tab not found`. The browser session IS active (the provider's app shows the account logged in), but clicking Authorize redirects to a re-auth/login URL instead of the OAuth callback.

**Root cause:** the provider enforces a **max auth-age**, the elapsed time since the account last performed a real sign-in, in addition to activity-age, before granting the OAuth scopes. A session-keepalive script slides the activity clock but cannot reset the auth-age clock. When auth-age exceeds the threshold, the consent flow forces a full logout regardless of session freshness.
...
**Why automation cannot always self-heal:** after the forced logout, the OAuth grant page may land on a federated "Continue with <provider>" button whose popup the browser blocks for extension-synthetic clicks (it requires a real user-activation gesture). The script then cannot complete the re-login loop unattended.

**Recovery:** one-time manual action, sign the account out of and back into the controlling browser. The browser session then satisfies the auth-age check and nightly relogins resume.

**A refresh token has its own deadline, so an auth-age failure is a countdown, not a steady state.** It is tempting to conclude there is "no user-facing impact during the failure window" because containers keep running and their access tokens auto-refresh. That is true for days, not weeks. A refresh token carries **its own expiry** (`refreshTokenExpiresAt`): roughly 28-30 days, reset only by a successful interactive rotation and NOT extended by ordinary refreshes. That is why breaking rotation did not surface for a month: the fleet had four weeks of runway and spent all of it silently. Any bridge that idles past that window wakes up unable to refresh; the CLI then writes the credential file back with **empty-string** `accessToken`/`refreshToken` and `expiresAt: 0`, and the health endpoint reports `auth: "failed", "OAuth session expired and could not be refreshed"`. Nothing automated can recover that state, because minting a new refresh token requires the interactive login that auth-age is blocking. Measured after 11 nights of un-repaired auth-age failure: 7 of 9 containers dead, each with a blanked credential and a `refreshTokenExpiresAt` in the past. The two survivors were the two whose chains never missed a refresh.

**Rule:** treat an auth-age failure as a deadline. Build the alarm for it: a daily monitor that reads `refreshTokenExpiresAt` for the host and every running container, warning under 20 days and paging under 10. That one field measures both how long until a credential is unrecoverable and how long rotation has been failing, and it does not care *why* rotation stopped, so it also catches the quiet skip (a pre-flight that exits 0 when the browser is asleep is right for one night and fatal over a fortnight). Also make relogin scripts close the tabs they open: leftover error pages from run N become the bug in run N+1, benign on night one and an outage by week two. The alarm that matters is the blank-credential page, not the nightly relogin email.
...
**Four stacked defects that made this look like auth-age when it was not.** The automation was eventually made to clear the wall unattended, and the fleet was restored without any human sign-in. Four separate defects had stacked up, each masked by the one in front of it:

1. **The caller's env was being overwritten by a sourced file.** Moving webhooks into a dotenv file and adding `set -a; . "$HOME/.env"; set +a` to the browser-driving scripts also re-defined `BROWSER_AGENT_KEY` (the main profile). `set -a` plus source lets the **file beat the caller**. Every alt rotation is invoked as `BROWSER_AGENT_KEY="$BROWSER_AGENT_KEY_ALT" ...`, so from that day each one drove the **main** browser profile: it opened an alt-account consent URL in a browser signed in as the other account, and the account chooser it reached never contained the expected address (`"clicked": false, "error": "not found"`). Every "the alt session is logged out" alert described a profile the run never touched. **Rule:** a script that sources an env file after receiving env from its caller must re-assert the caller's values, or the file silently wins.
2. **The chooser popup was invisible to the tab registry.** A browser-extension release added the identity provider's domain to the manifest's `exclude_matches`. The relay's registry is fed by content-script heartbeats, so from that day that tab could never appear in the tab listing, and the entire account-chooser branch was unreachable code. Fixed by adding a verb that queries tabs via the service worker instead.
3. **Origin containment hard-denied the clicks.** Same release. The sanctioned opt-in is an explicit "allow sensitive" flag, paired with an allowlist so it re-narrows to the one domain.
...
Bringing a second account to parity surfaced three further traps, all worth building into any shared library: the identity provider **versions** its consent paths (two profiles got different path versions on the same day, and the matcher knew neither the new version nor any future number), a **stale** tab on that domain hijacks the loop forever (a leftover error page with no buttons classified as consent, clicked nothing, every round, while the real wall was never touched again), and an unclassifiable page needs an **exit** (an "other" state with no clearing branch "settled" the same page twelve times logging nothing but the word). Rules: match provider states by keyword not by fixed path; snapshot the tab ids at entry and ignore pre-existing ones, since a popup you did not open is not yours to drive; and always log the URL of a page you could not classify.

Also fixed: the consent-tab matcher required a literal `oauth/authorize`, but the provider redirects to `/login?...returnTo=%2Foauth%2Fauthorize`, where it is percent-encoded, so losing the race against the redirect produced "consent tab not found - is the browser signed into the right account?" about a tab sitting right there. Match the **decoded** URL. And on one consent page, Cancel and Continue share the same `jsname` attribute: select the primary button by visible label, never by an opaque attribute.

**Two traps that hid all of this for six weeks:**

1. **A per-key operation gated on a global signal reports on the wrong thing.** The browser relay's tab-listing endpoint is a single un-keyed registry: every API key sees the union of tabs from **both** browser profiles, while *commands* route per key to one profile's extension. So an `ensure <url>` call would match a URL, hand back the **other** profile's tab, and every later read on that id timed out. The session-keepalive was therefore reading the main profile's tab and certifying its session daily, logging "session appears alive" while the other session aged out, tripped auth-age, and finally logged itself out. Its content check was correct; it was pointed at the wrong browser. The same bug sat in the relogin pre-flight, which gated on a global `connectedClients` count: an awake main-profile browser satisfied it while the other extension was absent. Fixed by probing ownership with a keyed `ping` and gating on a per-key status endpoint. **Rule:** if a check is per-profile/per-key/per-tenant, its signal must be too, and "I could not read it" is never evidence of health.
...
The CLI binary may not be where you assume. On Debian-family hosts installed via the system package path it is `/usr/bin/claude`, **not** `/usr/local/bin/claude`. Using the wrong fallback path causes silent `[Errno 2] No such file or directory` failures that drop all AI processing without any obvious error in service logs.

Observed: an error handler had `CLAUDE_BIN = "/usr/local/bin/claude"` hardcoded as the default. All invocations from it failed silently.

**Rule:** verify with `command -v claude` on the target host, and always prefer the `CLAUDE_BIN` env var over hardcoding so deployments with non-standard paths can override it:
...
Observed: a bridge server returned rate limit text as a "completed" deliverable. Jobs were marked successful with useless content because the bridge only checked exit code, not output content.
...
Observed: three autonomous runners plus a verification script all had this bug. Zero failure alerts had ever fired across ~1,000 combined runs; a 45-minute run timed out with no log entry, no state write, and a reused run ID the next day; the verify gate passed proven test failures for weeks.
...
Classify Stop hooks in tiers: Tier 1 observation (log only), Tier 2 verification (reads state, no model calls), Tier 3 Claude-invoking. Every Tier 3 hook must go through a shared guard library providing an env var circuit breaker, a PID lockfile, and a per-hour rate limiter.

Observed: a Stop hook ran a session scorer (`claude -p --model haiku`) on every session exit. The scorer's own session exit re-triggered the hook. Result: 4,888 recursive sessions in one day, 199M tokens (78% of the week's usage). Fixed by adding an env var guard plus a content pattern match, then standardizing via the guard library.

## Concurrent Sessions in One Checkout

Two distinct problems, often conflated:

- **Problem A, shared tree:** two sessions editing the same checkout stomp each other's branch state and working files. Fix: a git worktree per session (`git -C <repo> worktree add /tmp/<label> -b <branch>`).
- **Problem B, singletons:** two sessions racing a resource that admits only one writer (a state file, a crontab, a deploy). Fix: a real lock (`flock`), not a "check then act" guard.

Diagnostic order when something keeps reverting: check `git branch --show-current` in the shared checkout first, then look for a second process holding the file, then look at whatever regenerates the artifact.

Note the meta-lesson: this topic once grew three separate homes in a single afternoon, written by three sessions that could not see each other, which is the problem itself in miniature. Keep one canonical location and point everything else at it.
...
5. **Report every outcome, including failure.** Silence must never be the success signal. Notify on success, failure, AND skip.
...
**Why:** An agent asserted a user had applied to a role because prep materials existed in cloud storage. The application was never actually submitted, and incorrect context was passed on to a third party as a result.

**How to verify:**
- **Applications/emails:** Check the sent folder for a confirmation
...
**When writing a health monitor or watchdog that scans processes, always skip the monitor's own process.**
...
Observed: an error-handling daemon scanned all process-manager processes including itself. A dedup window increase (1h -> 24h) was also needed to prevent repeated self-triggering within a single incident window.

**Rule:** Any monitoring daemon that enumerates processes (`pm2 jlist` or equivalent) must exclude its own process name from health checks.

### Log Artifacts and ANSI Codes in Error Handlers

Two related patterns that cause error handler crash loops or notification floods when reading process-manager log output:
...
A free tier an automated runner depends on can be withdrawn without warning, and the failure lands *before* the first request, so no flag or argument change suppresses it. When it happens, every consumer that shells out to that CLI silently errors, and cron wrappers that ignore exit codes never alert.

**Rules:**
- Any shadow runner or fallback path built on a free-tier CLI needs a second provider configured, or its absence is a silent capability loss.
- Probe before relying on it in automation: run the CLI's smallest possible invocation and check both exit code and stderr, rather than assuming availability.
- When a fallback path is documented elsewhere in your runbooks, re-verify it: the "fallback" is exactly the path nobody exercises until the primary fails.

## Shared Poller Resource Gates Must Be Scoped to the Executing Machine

When a poller or executor dispatches jobs to BOTH local and remote workers, any resource gate based on the local machine's resources (RAM, CPU, disk) must NOT fire for remote jobs: those jobs consume zero local resources.

Observed: a multi-worker dispatcher had ONE memory watchdog for all jobs; when the dispatch host's `MemAvailable < 100MB`, it killed the tracked PID. For remote SSH jobs the tracked PID was only the SSH client on the dispatch host; the real work ran on the remote worker using zero local memory. Local memory pressure was killing healthy offloaded sessions. Symptom: the log showed "Memory watchdog: only 93MB available, killing process PID" while dispatch counts showed only remote dispatches and zero local ones.
...
**Symptom:** the primary working copy (not a worktree) is checked out on a feature branch whose PR has already been merged. The local branch is stranded and local `main`/`production` silently falls behind `origin/main`, sometimes by several commits, with no error surfaced anywhere.
...
**Prevention:** any session opening a PR from a main checkout should instead use `git -C <repo> worktree add /tmp/<label> -b <branch>`, work there, and leave the main checkout untouched on its default branch throughout.
...
**Fix:** write the script/patch to a LOCAL file and `scp` it, then run `ssh host 'python3 /tmp/file.py'`. Always `node --check` / syntax-validate on the remote host BEFORE restarting, and keep a `.bak` to restore.

## Audit CLI Version on Every Host, and Pin Fan-Out/Search Defaults Before Upgrading

Version drift silently keeps already-fixed reliability bugs in play, and headless hosts drift worst because nobody watches their startup banner. A real audit found the interactive host 19 versions behind and a server host 8 behind, both still exposed to fixed bugs: truncated-tool-output results retained in memory for the rest of the session, stream-json output truncated at exit for slow-reading pipeline consumers (which makes a headless job report success with the tail of its response missing), and quadratic message-normalization stalls in long sessions.
...
3. **`auth=pending` on a health endpoint immediately after a rebuild is expected, not a failure.** If the server checks auth on a 30-minute interval with the first check ~60s after start, wait for the "checked N seconds ago" field to be populated before judging.

Credentials survive a rebuild when they live in a named volume mounted over the CLI's config dir, not in the image, so `docker compose build && up -d` preserves them and no re-OAuth is needed.

### The Claude Code OS Sandbox Is Not Usable Inside Docker or on WSL2

**Containers cannot use it.** The sandbox is enforced by bubblewrap, which requires unprivileged user namespaces. Inside a typical container `bwrap` fails with `Creating new namespace failed: Operation not permitted`, including the weaker variant that binds the existing `/proc`. The host kernel is usually NOT the blocker (check `/proc/sys/user/max_user_namespaces` inside the container); Docker's default seccomp profile blocks `CLONE_NEWUSER`. Enabling it would require running with `--privileged`, `--cap-add SYS_ADMIN`, or `seccomp=unconfined`.

That trade is backwards: it punches a hole in the OUTER isolation boundary in order to add an inner one, on containers whose entire purpose is isolating untrusted public input. **For such containers, the Docker container IS the sandbox.** Do not weaken it to add a nested sandbox. The "weaker nested sandbox" option does not rescue this: it addresses a container that cannot mount a fresh `/proc`, not one forbidden from creating namespaces at all.

**WSL2 cannot use it either, for a different reason.** Sandboxed commands cannot launch Windows binaries or anything under `/mnt/c/`. If your primary working directory lives on the Windows filesystem and Windows interop is routine, enabling the sandbox breaks that wholesale; `docker` is separately documented as sandbox-incompatible.

**Where the real mitigation lives instead.** If the exposure that motivated this was a broad tool allowance (say `Bash(curl:*)`) on untrusted public input, and the OS-level sandbox is unavailable, the controls that DO apply are: a narrow `--allowedTools` list, account isolation, the container boundary itself, and an output scrubber. Harden those.
...
Restart storms come from a health check with fewer states than reality. An auth-refresh watchdog had two: `ok` and "not ok", where "not ok" meant stale bind-mounted credentials and the remedy was recreating the container. Being out of quota also reads as "not ok", so it inherited a remedy that cannot return quota: 1273 container recreates across six container logs, tearing down and rebuilding every 10 minutes for the length of each limit window, plus an alert telling the operator to re-login for a condition login cannot fix.
...
Observed: a dependency fleet was down ~60 minutes. Recovery ran every 5 min with MAX_ATTEMPTS=2, so both attempts were spent inside the first ten minutes. A user's job was dead for 5 hours with a useless error until a human reported it.
...
A supervisor that terminates a job from outside the code path that reaps it leaves the reaper with only one observable: the process is gone. Absent an explicit reason, "gone" is indistinguishable from "finished", so the system reports a clean completion carrying whatever partial work existed. A stall watchdog SIGKILLed a job at 6 minutes of silence; the completion poller saw a dead pid with `killReason=null` and resolved normally, so the user saw 21 minutes of tool narration under an ordinary completion footer and no answer.
...
## Restart-Count Thresholds: `restart_time` Is Cumulative, `unstable_restarts` Resets

Fixing the SSH bug above unmasked a second, more consequential bug in the same script: a `STORM_THRESHOLD=5` check compared against `.pm2_env.restart_time`, the process manager's cumulative lifetime restart count, which never resets. Any long-uptime stable production process trivially exceeds 5 (observed: 79, 182, and 233 restarts, all with `unstable_restarts=0`, genuinely stable). A dry-run then reported "STORM DETECTED" for a process that was not flapping at all.
...
Three fixes shipped together, all worth copying:
- Swap the jq field at both the pre-rollback storm check and the post-rollback stability check.
- Add a hard process-name allowlist guard INSIDE the script (not just in the calling agent's textual instructions), so a non-dry-run invocation against any non-staging process name is refused before any stop/rollback command runs.
- Normalize jq's empty-output case: `select()` with no match emits nothing at all, so `// 0` never fires; an empty string then trips a `set -e` integer-test error that falls through to a false STORM DETECTED. Normalize to 0 explicitly.

**Lesson:** a script that reads a process-manager field by name is only as correct as the field choice. `restart_time` and `unstable_restarts` sound interchangeable but have opposite reset semantics; always verify against a known-stable long-uptime process before trusting a restart-count threshold.

## A Two-Signal Liveness Check Is Only as Strong as Its Weaker Signal

A daily session-liveness keepalive reported "session appears alive" every single day for 11+ consecutive nights while the session was actually logged out. Nothing alerted: the check's own log line stayed uniformly positive throughout. Downstream, every service depending on that session to mint fresh OAuth refresh tokens degraded independently as its own token separately expired; by the time anyone looked, most were silently dead, with no single alert pointing at the root cause.
...
A daily discovery job sent mail on completion and nothing at all on failure (one channel got a `[FAILED]` post; the inbox got nothing). So "the run broke" and "the run found nothing new" arrived as the same event: an empty inbox. An upstream outage ran four full days before it was noticed, discovered by remembering the absence, not by being told.
...
**Symptom:** interactive sessions all end at once with no warning, each offering `claude --resume <id>`. This looks like a client-side crash or a closeout hook firing; check both before assuming either. A host-OS sleep/reboot can be ruled out independently via the OS's own uptime/power-event log.
...
To auto-start a WSL distro at Windows boot (so supervised services recover without a login), a Task Scheduler task with an AtStartup trigger MUST run as the Windows user who owns the distro, using LogonType S4U (runs before login, no stored password). Running it as `NT AUTHORITY\SYSTEM` FAILS: WSL distros are registered per-Windows-user (`HKCU\...\Lxss`), so SYSTEM has no distro registered and `wsl -d <Distro> ...` returns `LastTaskResult 0xFFFFFFFF`.

**Fix:** `New-ScheduledTaskPrincipal -UserId '<HOST>\<windows-user>' -LogonType S4U -RunLevel Highest`. Note the Windows user often differs from the Linux user; use `wsl -d <Distro> -u <linux-user> -e /bin/true` as the action.
...
Related: an unattended OS update can reboot the host at any hour, which is NOT the OOM pattern above. Distinguish via the OS's own power-event log before diagnosing. Without the boot task, WSL stays down until someone logs in, which was a ~6 hour outage in one observed case.

## `systemctl --user` Can Be Permanently Broken on a WSL Host (No User D-Bus)

On some WSL hosts, `systemctl --user` ALWAYS fails with `Failed to connect to bus: No such file or directory`: `/run/user/<uid>/bus` and `/run/user/<uid>/systemd/private` are absent even though the `systemd --user` manager IS running and supervising units with `linger=yes`. So any cron/automation that calls `systemctl --user restart|start|stop` is SILENTLY broken there (it logs the bus error and does nothing).

Observed: a self-heal script restarted a reverse-tunnel service via `systemctl --user restart <unit>`, which never worked, so dead reverse-forward ports stayed dead for minutes on every tunnel drop and the jobs depending on them got cut off.

**Bus-free fix pattern:** to restart a supervised user unit, kill its main process and let `Restart=always` respawn it (the manager's internal restart logic needs no D-Bus): `pkill -TERM -f <exec-pattern>`. Same root cause as the note above that `systemd-run --user` does not work on such a box: use the SYSTEM manager (`sudo systemd-run --uid`) or process-kill plus `Restart=always`, never `systemctl --user` in headless/cron contexts.

**Rule:** verify a `systemctl --user` call actually works on a given host before trusting it: run it once and check for the bus error.

Second contributing fix in the same incident: an sshd holding dropped reverse-forward ports for up to ~6 minutes (`ClientAliveInterval 120` x `CountMax 3`) blocked rebind under `ExitOnForwardFailure=yes`. Lowering to 30/2 via a drop-in in `/etc/ssh/sshd_config.d/` let the ports self-reap in ~60s.
